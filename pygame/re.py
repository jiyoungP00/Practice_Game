...
def gameStart():
    ...
    rect_ball = pygame.Rect(x_pos_ball, y_pos_ball,
                            size_r_ball*2, size_r_ball*2)
    ...
    for i in range(10):
        for j in range(3):
            ...
            color_block[i].append((random.randrange(255), random.randrange(
                150, 255), random.randrange(150, 255)))                         # 컬러값을 랜덤으로 뽑음
    ...
    while play:
        ...
        # 페달 그리기
        pygame.draw.rect(background, (255, 255, 0), rect_ped)
        ...
        pygame.draw.circle(background, (255, 0, 255),
        ...
        # 블록 생성 (for문으로 10개*3층 만들기)
        for i in range(10):
            for j in range(3):
                if rect_block[i][j]:
                    pygame.draw.rect(
                        background, color_block[i][j], rect_block[i][j])
                    rect_block[i][j].topleft = (
                        i*size_w_block, j*size_h_block)
